...
- **Inférence compatible OpenAI** — Appeler `chat/completions` et `embeddings` en réutilisant le SDK `openai` (changement de `base_url`) ; reclasser des résultats avec `rerank`, qui s'appelle en HTTP direct (le SDK OpenAI n'a pas de ressource `rerank`)
