...
> - [`skills/`](skills/) — skills **officielles**, portées par le département IAE de la DINUM, adossées à des standards de l'État (DSFR, RGAA, ANSSI, data.gouv.fr, La Suite).
> - [`skills/.experimental/`](skills/.experimental/) ⚠️ — skills **expérimentales**, non portées par la DINUM, consommant des outils tiers non audités. À utiliser avec précaution. Voir l'[avertissement](skills/.experimental/README.md).
...
| [**anssi-guides**](skills/anssi-guides/) | Trouver et consulter le bon guide dans le catalogue complet de l'ANSSI (128 guides) — aiguillage et citation, complémentaire de `securite-developpement` |
...
Non portées par la DINUM, dépendances tierces non auditées, pas de garantie de maintenance. [Lire l'avertissement complet](skills/.experimental/README.md) avant adoption.
...
```bash
# Installer toutes les skills (officielles + expérimentales)
...
> Le sous-dossier `skills/.experimental/` est une convention reconnue par la CLI : les skills expérimentales sont installables au même titre que les skills officielles. Vérifier leur statut avant adoption (cf. [avertissement](skills/.experimental/README.md)).
...
│   ├── SKILL.md
│   └── references/
│       └── catalogue.md          # les 128 guides FR du catalogue ANSSI + méthode de re-scan
├── datagouv-apis/
│   ├── README.md
│   └── SKILL.md
